...
class Channel {
  constructor(props) {
    ...
    if (props.synchronizationStatus)
    ...
    this._channelSynchronizationStatusChangedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, status }) => {
        if (channelSid === this.sid && this.onSynchronizationStatusChanged)
          this.onSynchronizationStatusChanged(status);
      }
    );
    ...
    this._channelMemberJoinedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onMemberJoined)
          this.onMemberJoined(new Member(member, channelSid));
      }
    );
    ...
    this._channelMemberChangedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onMemberChanged)
          this.onMemberChanged(new Member(member, channelSid));
      }
    );
    ...
    this._channelMemberLeftSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onMemberLeft)
          this.onMemberLeft(new Member(member, channelSid));
      }
    );
    ...
    this._channelMessageAddedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, message }) => {
        if (channelSid === this.sid && this.onMessageAdded)
          this.onMessageAdded(new Message(message, channelSid));
      }
    );
    ...
    this._channelMessageChangedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, message }) => {
        if (channelSid === this.sid && this.onMessageChanged)
          this.onMessageChanged(new Message(message, channelSid));
      }
    );
    ...
    this._channelMessageDeletedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, message }) => {
        if (channelSid === this.sid && this.onMessageDeleted)
          this.onMessageDeleted(new Message(message, channelSid));
      }
    );
    ...
    this._typingChannelStartedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onTypingStarted)
          this.onTypingStarted(new Member(member, channelSid));
      }
    );
    ...
    this._typingChannelEndedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (channelSid === this.sid && this.onTypingEnded)
          this.onTypingEnded(new Member(member, channelSid));
      }
    );
    ...
    this._toastReceivedChannelSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, message }) => {
        if (channelSid === this.sid && this.onToastReceived)
          this.onToastReceived(new Message(message));
      }
    );
      ...
      ({ channelSid, updated, userDescriptor }) => {
        if (channelSid === this.sid && this.onMemberUserInfoUpdated) {
          this.onMemberUserInfoUpdated({
            updated,
            userDescriptor: new UserDescriptor(userDescriptor),
          });
        }
      }
    );
  ...
  getMember(identity) {
    return TwilioChatChannels.getMember(this.sid, identity).then(
      member => new Member(member, this.sid)
    );
  }
  ...
  async getMembers() {
    ...
    if (Platform.OS === 'ios') {
      return TwilioChatMembers.getMembers(this.sid).then(
        ({ sid, type, paginator }) => new Paginator(this.sid, type, paginator)
      );
    }
  ...
  getMessages(count = 10) {
    return TwilioChatMessages.getLastMessages(this.sid, count).then(messages =>
      messages.map(message => new Message(message, this.sid))
    );
  }

  getMessagesBefore(index, count) {
    return TwilioChatMessages.getMessagesBefore(
      this.sid,
      index,
      count
  ...
  getMessagesAfter(index, count) {
    return TwilioChatMessages.getMessagesAfter(
      this.sid,
      index,
      count
    ).then(messages => messages.map(message => new Message(message, this.sid)));
  ...
  getMessage(index) {
    return TwilioChatMessages.getMessage(this.sid, index).then(
      message => new Message(message, this.sid)
    );
  }

  getMessageForConsumption(index) {
    if (Platform.OS !== 'ios')
      console.warn('getMessageForConsumption is only available on iOS.');
    else {
      return TwilioChatMessages.messageForConsumptionIndex(
        this.sid,
        index
  ...
  sendAttributedMessage(body, attributes) {
    return TwilioChatMessages.sendAttributedMessage(this.sid, body, attributes);
  }
